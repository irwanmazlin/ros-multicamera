...
//Image to blob
void ObjectDetectionSSD::frame_to_blob(const cv::Mat& image,InferRequest::Ptr& analysis,const std::string& descriptor) {
    analysis->SetBlob(descriptor, mat_to_blob(image));
}

//Callback: a new RGB image is arrived
void ObjectDetectionSSD::imageCallback(const sensor_msgs::Image::ConstPtr& image_msg){
...
void ObjectDetectionSSD::infoCallback(const sensor_msgs::CameraInfo::ConstPtr& info_msg){
    fx=info_msg->K[0];
    fy=info_msg->K[4];
    cx=info_msg->K[2];
    cy=info_msg->K[5];        
...
void ObjectDetectionSSD::depthCallback(const sensor_msgs::Image::ConstPtr& depth_msg){
...
void ObjectDetectionSSD::run()
{
    ...
    try{
        ...
        // getParameters(ros::NodeHandle n);
        //Handle creation
        ros::NodeHandle n("~");

        getParameters(n);
        ROS_INFO("SAMPAI DISINI");
        //ROS subscribers
        // ros::Subscriber image_sub = n.subscribe(image_input,1,&ObjectDetectionSSD::imageCallback, this);
        ros::Subscriber image_sub = n.subscribe(image_input,1,&ObjectDetectionSSD::imageCallback, this);
        ...
        if (output_as_list){
            result_pub = n.advertise<detecttrack::Objects>(objects_output,1);
        }
        ...
        if (depth_analysis){
            depth_sub = n.subscribe("/object_detection/input_depth",1,&ObjectDetectionSSD::depthCallback, this);
            camerainfo_sub = n.subscribe("/object_detection/camera_info",1,&ObjectDetectionSSD::infoCallback, this);
            if (output_markers||output_markerslabel){
                marker_pub = n.advertise<visualization_msgs::MarkerArray>("/object_detection/markers", 1);
            }
            if (output_boxlist){
                boxlist_pub = n.advertise<detecttrack::ObjectBoxList>("/object_detection/box_list", 1);
            }
        }
        ...
        */
        ...
        InferRequest::Ptr engine_curr = model_network.CreateInferRequestPtr();     //         ROS_WARN("infer request");
...
bool ObjectDetectionSSD::getParameters(ros::NodeHandle n)
{
    ROS_INFO("[ObjectDetectionSSD] Getting parameters ...");
    n.param("image_input", image_input, std::string("/object_detection/input_image"));
    ROS_INFO("Set topic_image_input to: %s", image_input.c_str());
    n.param("image_output", image_output, std::string("/object_detection/output_image"));
    ROS_INFO("Set topic_image_output to: %s", image_output.c_str());
    n.param("objects_output", objects_output, std::string("/object_detection/results"));
